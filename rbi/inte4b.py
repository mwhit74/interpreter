...
class Interpreter(object):
    ...
    def expr2(self):
        ...
        while self.current_token.type is not EOF:
            if self.current_token.type is PLUS:
                self.check_type(PLUS)
                result += self.expr1()
            elif self.current_token.type is MINUS:
    ...
    def expr1(self):
        ...
        expr2 : expr1((PLUS|MINUS)expr1)*
        factor : INTEGER
        """
        result = self.factor()
        while self.current_token.type not in (PLUS, MINUS, EOF):
